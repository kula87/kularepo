...
Logging Strategy Guidelines provide best practices to create efficient logging systems, ensuring consistency, security, and easy analysis throughout an application or organization.

## 1. Logging Levels

Logging levels are crucial for categorizing log messages by severity and importance, aiding developers and support teams in filtering and analyzing logs more efficiently.

- **DEBUG**: Variable values, internal states, execution flow
- **INFO**: Application start-up/shutdown, significant events/ops, No sensitive information
- **WARN**: Configuration issues, deprecated API usage, Performance-related concerns
- **ERROR**: Exception details, stack traces, relevant context data
- **FATAL**: Critical issues that require immediate attention

## 2. Logging Pattern and Structure
...
### 2.1.1 Timestamp (ISO 8601 format)
...
| `yyyy-MM-ddTHH:mm:ss.SSSZ` | ISO 8601 format with milliseconds and a time zone offset in hours | `2023-05-08T14:30:15.123+09:00` |


Including local time in logs isn't advised, as it may cause confusion when correlating events. However, if needed, add a field like "japan_time" to log entries.

{
  "timestamp": "2023-05-08T14:35:10.123Z",
  "japan_time": "2023-05-08T23:35:10.123+09:00",
  "level": "INFO",
...
### 2.1.3 Application or module name
...
### 2.1.4 IP address (if applicable)
...
### 2.1.7.1 Logging patterns
Aspect | Sample Pattern | Summary Description
-------|---------------|-------------------
API Request | `{"timestamp": "2023-05-08T15:45:30.123Z", "level": "INFO", "transactionId": "c1a89a5b-2075-4e72-a9a1-0e6cddbf0a21", "message": "GET /api/v1/users", "ipAddress": "192.168.1.100"}` | API GET request with transaction ID and IP address
...
These are general guidelines, and the most important aspect is to maintain consistency throughout the log messages and follow the conventions used within the project.

## 3. Logging Output
- JSON format
- Plain text format

## 4. Logging Storage Rules
Logging Storage Rules cover storage location, retention, rotation, and backup/archiving for effective log management.

### 4.1. Storage Location
The storage location is vital for log data security, accessibility, and manageability. Various options, each with pros and cons, exist for storing log data.

Table summarizing various storage locations for log data:

| Storage Location | Description                                  | Pros                           | Cons                         |
|------------------|----------------------------------------------|--------------------------------|------------------------------|
| Local Disk       | Store logs on the system where they're generated | Easy to set up, low cost     | Limited space, single point of failure |
| Centralized Log Server | Send logs to a dedicated server            | Simplified management, better security | Increased network traffic, setup complexity |
| Cloud-based Storage | Store logs on cloud platforms like AWS, GCP  | Scalable, high availability   | Costs, potential latency    |
| Database         | Store logs in a relational or NoSQL database   | Structured data, easy querying | Complexity, performance impact |
| Log Management Service | Use third-party services like Splunk, ELK | Scalable, feature-rich        | Costs, vendor lock-in       |


Here's a table summarizing the factors to consider when choosing a storage location for your logs:

| Factor              | Description                                                                                                         |
|---------------------|---------------------------------------------------------------------------------------------------------------------|
| Security            | Ensure the storage location provides adequate protection against unauthorized access, tampering, or loss of log data. |
| Accessibility       | The storage location should be easily accessible to developers, support teams, and log management tools.             |
| Scalability         | Choose a storage location that can handle your log data volume and scale as your logging needs grow.                 |
| Retention & Archiving | Consider log retention and archiving policies required by your organization or regulations, and ensure compatibility.  |
| Cost                | Factor in storage and maintenance costs, including hardware, software, and operational expenses.                     |
...
### 4.4. Backup and Archiving Policies
...
### 5.1. Exclude Critical or Sensitive Information
...
### 5.2. Sanitize log data to prevent log injection attacks
...
### 5.2.1 input validation to prevent log injection attacks
...
### 5.2.2 Encode or escape unsafe characters or strings in log data
...
### 5.2.6 Updates & security patches
...
## 7. Integration with Existing Tools (Splunk and Kibana)
